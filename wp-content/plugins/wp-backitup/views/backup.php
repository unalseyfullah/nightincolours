    ...
    if (null != $file_list && is_array($file_list)) {
        foreach ( $file_list as $file ) {
            ...
            if ( empty( $folder_name )  || $extension!='.zip' ) {
                WPBackItUp_LoggerV2::log_error( $debug_backup_view_log, __METHOD__, 'File does not appear to be a WP BackItUp backup archive:'. $file );
...
<div class="wrap">
  ...
  <div id="content">
    ...
    <div class="widget">
      ...
      <p> <?php _e('WP BackItUp premium customers can use these backup files to perform an automated restore of their site.', 'wp-backitup') ?></p>
      <p>
      ...
      if (!$license_active)
        echo '<p> * ' . sprintf(__('WP BackItUp lite customers may use these backup files to manually restore their site.  Please visit %s for manual restore instructions.', 'wp-backitup'), $this->get_anchor_with_utm('www.wpbackitup.com','documentation/restore/how-to-manually-restore-your-wordpress-database','backup','manual+restore')) .'</p>';
      ?>
    </div>
      ...
      <div class="widget">
          ...
          <div id="scheduled-backups" style="display: none;">
              ...
              if (!$license_active || 'expired'== $license_status)
                  echo '<p>* ' . sprintf(__('Scheduled backups are only available to WP BackItUp premium customers.  Please visit %s to get WP BackItUp risk free for 30 days.', 'wp-backitup'), $this->get_anchor_with_utm('www.wpbackitup.com','pricing-purchase','scheduled+backups','risk+free') ) . '</p>';
              ?>
          </div>
      </div>
    ...
    <form id = "download_backup" name = "download_backup" action="<?php echo get_admin_url(),"admin-post.php"; ?>" method="post">
      ...
      //Display restore note for lite customers
      if (!$license_active)
        echo '<p>* ' . sprintf(__('The automated restore feature is only available to WP BackItUp premium customers.  Please visit %s to get WP BackItUp risk free for 30 days.', 'wp-backitup'), $this->get_anchor_with_utm('www.wpbackitup.com','pricing-purchase','available+backups','risk+free')) . '</p>';
      ?>
    </div>		
  ...
  <div id="sidebar">
          ...
          <div class="widget">
              ...
              <?php if ($this->successful_backup_count()<1) : ?>
                  <p><?php _e('Welcome to WP BackItUp!', 'wp-backitup') ?><br/>  <?php _e('The simplest way to backup your WordPress site.', 'wp-backitup') ?></p>
    ...
    <?php if (false===$license_active) : ?>
        <?php if (false===$is_lite_registered) : ?>
            <div class="widget">
                <h3 class="promo"><span><?php _e('Register WP BackItUp', 'wp-backitup'); ?></span></h3>
